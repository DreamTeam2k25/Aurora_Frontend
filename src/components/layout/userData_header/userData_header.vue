...
watch(()=> studentsStore.studentExists, async (newValue) => {
...
<section class="w-dvw h-dvh fixed overflow-x-hidden" v-if="studentsStore.open || animateClose">
    ...
    <div v-if="studentId" class="flex flex-col pt-20 gap-24 items-center">
        <div v-for="(data,index) in returnUserInformation" :key="index" class="flex flex-col items-center">
            <p class="text-[#0455BF]">{{data.title}}</p>
            <p class="text-[#6B6B6B]">{{ data.content }}</p>
        </div>
    </div>
    <div v-else class="flex w-full justify-center">
    <GlobalButton class="mt-5" @evento="router.push('/auth-student'),studentsStore.state.open = false" content="Torne-se um estudante" width="w-3/4"/>
    </div>
...
<div v-else class="w-full h-full flex justify-center items-center flex-col gap-5">
    ...
    <div class="rounded-full p-5 loading-card"></div>

</div>
    
    </div>

    <section @click="studentsStore.state.open = false" :class="`absolute w-dvw right-0 h-dvh z-30 bg-[#4d6f8d80] ${studentsStore.open ? 'open' : 'close'}`"></section>
</section>
</template>
