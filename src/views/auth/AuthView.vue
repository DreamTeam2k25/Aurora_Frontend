  ...
  if (authStore.methodByLink) {
    ...
    const response = await authStore
      ...
      messageContent.content = 'Sucesso ao Criar Usuário'
          ...
          appearMessage(true, '/auth-student', resultRequisition, message)
          return response
        } catch (error) {  
...
<template>
  <MessageConfirmation
    ...
    :icon="resultRequisition ? 'mdi mdi-check-decagram' : 'mdi mdi-alert-circle'"
    v-if="message"
  />
  <main class="max-h-dvh h-dvh relative flex overflow-hidden">
    ...
    <div class="flex absolute min-w-dvw max-lg:hidden">
      <AuthMarketingText
        :has-before-content="authStore.methodByLink"
        :is-login="authStore.methodByLink"
        :first-short-text="textContent.firstShortText.content"
        :first-main-text="textContent.firstMainText.content"
        :second-main-text="textContent.secondMainText.content"
        :third-main-text="textContent.thirdMainText.content"
        :second-short-text="textContent.secondShortText.content"
        :class="` z-50 ${firstEntrance == 0 && !authStore.methodByLink ? 'bottom-[-33rem] right-[5rem] register' : firstEntrance == 0 ? 'bottom-[-35rem] left-[5rem]' : !authStore.methodByLink ? 'bottom-[-33rem] right-[5rem] register' : 'bottom-[-35rem] left-[5rem] login'} `"
      />
      <div
      ...
      >
        <div
          :class="`${firstEntrance == 0 && !authStore.methodByLink ? 'wave-animation-back' : firstEntrance == 0 ? 'wave' : !authStore.methodByLink ? 'wave-animation-back' : 'wave-animation'}`"
        ></div>
      </div>
    </div>
    <div class="flex items-center relative w-[100vw] justify-center">
      <AuthInput
        @auth="doAuth"
        :button_text="authStore.methodByLink ? 'Entrar' : 'Cadastrar-se'"
        :alternative_text="authStore.methodByLink ? 'Não possui cadastro?' : 'Já possui cadastro? '"
        :alternative_link="authStore.methodByLink ? 'Cadastre-se' : 'Faça login'"
        :auth-method="authStore.methodByLink ? 'Login' : 'Registro'"
        :fields="authStore.methodByLink ? ['email', 'senha'] : ['email', 'senha', 'nome']"
